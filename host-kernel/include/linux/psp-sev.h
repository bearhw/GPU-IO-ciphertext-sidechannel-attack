...
struct sev_data_snp_page_swap_out {
	u64 gctx_paddr;				/* In */
	u64 src_paddr;				/* In */
	u64 dst_paddr;				/* In */
	u64 mdata_paddr;			/* In */
	...
	u8 page_type:2;				/* In */
	u8 rsvd1:1;
	u8 root_mdata_en:1;			/* In */
	u64 rsvd2:59;
} __packed;

struct sev_data_snp_page_swap_in {
	u64 gctx_paddr;				/* In */
	u64 src_paddr;				/* In */
	u64 dst_paddr;				/* In */
	u64 mdata_paddr;			/* In */
	...
	u8 page_size:1;				/* In */
	u8 page_type:2;				/* In */
	u8 swap_in_place:1;			/* In */
	u8 root_mdata_en:1;			/* In */
	u64 rsvd2:59;
} __packed;

struct sev_data_snp_page_move {
	u64 gctx_paddr;				/* In */
	u8 page_size:1;				/* In */
	u32 rsvd1:31;
	u32 rsvd2;
...
/**
 * snp_guest_page_move - perform SEV SNP_PAGE_MOVE command
 * 
 * TODO: Add more documentation for the command
 */
int snp_guest_page_move(u64 gctx_pfn, u64 src_pfn, u64 dst_pfn, int *error,
			u64 src_gpa, int asid);
...
static inline int snp_guest_page_move(u64 gctx_pfn, u64 src_pfn, u64 dst_pfn,
				      int *error, u64 src_gpa, int asid)
{
	return -ENODEV;
}
