...
static struct rmpentry *__snp_lookup_rmpentry(u64 pfn, int *level)
{
	...
	if (!pfn_valid(pfn)) {
		printk("Invalid pfn 0x%llx\n", pfn);
		return ERR_PTR(-EINVAL);
	}

	if (!cpu_feature_enabled(X86_FEATURE_SEV_SNP)) {
		printk("SEV-SNP is not enabled\n");
		return ERR_PTR(-ENXIO);
	}

	vaddr = rmptable_start + rmptable_page_offset(paddr);
	if (unlikely(vaddr > rmptable_end)) {
		printk("RMP entry not found for pfn 0x%llx\n", pfn);
		return ERR_PTR(-ENXIO);
	}
...
int get_rmpentry_asid(u64 pfn)
{
	...
	return rmpentry_asid(e);
}
EXPORT_SYMBOL_GPL(get_rmpentry_asid);

// Get assigned of a pfn
int get_rmpentry_assigned(u64 pfn)
{
	struct rmpentry *e;
	int level;
...
EXPORT_SYMBOL_GPL(get_rmpentry_assigned);
...
retry:
	 ...
	 *
	 * Swap+Compare hack: always restore, even when assigning (the
	 * original condition was `!ret && !val->assigned`).  We also
	 * restore on FAILURE when val->assigned was set, because
	 * invalid_direct_map() already ran above — leaving the entry
	 * removed on a failed RMPUPDATE leaks the direct-map mapping
	 * and any later kernel access to that pfn page-faults.
	 */
	if (!ret || val->assigned) {
...
int rmp_make_pre_swap(u64 pfn)
{
	...
	// printk("rmp_make_pre_swap: pfn=%llx\n", pfn);
	...
	if (rmpentry_immutable(e))
		printk("Warning: rmpentry is immutable already: %llx\n", pfn);
	
	val.gpa = rmpentry_gpa(e) << PAGE_SHIFT;
	val.immutable = 1;
	val.assigned = rmpentry_assigned(e);
	val.pagesize = rmpentry_pagesize(e);
	val.asid = rmpentry_asid(e);

	// printk("rmpentry_gpa(e)=%llx, rmpentry_assigned(e)=%d, rmpentry_pagesize(e)=%d, rmpentry_asid(e)=%d\n", val.gpa, val.assigned, val.pagesize, val.asid);
	// printk("val.gpa=%llx, val.immutable=%d, val.assigned=%d, val.pagesize=%d, val.asid=%d\n", val.gpa, val.immutable, val.assigned, val.pagesize, val.asid);

	return rmpupdate(pfn, (struct rmpupdate *)&val);
}
EXPORT_SYMBOL_GPL(rmp_make_pre_swap);
...
int snp_free_guest_invalid_page(struct page *page)
{
	...
	if (page == NULL) {
		printk(KERN_ERR
		       "snp_free_guest_invalid_page: Invalid page=NULL\n");
		return -EINVAL;
	}
	...
	if (!pfn_valid(pfn)) {
		printk(KERN_ERR
		       "snp_free_guest_invalid_page: Invalid pfn=0x%llx\n",
		       pfn);
		return -EINVAL;
	}

	// make sure it is in the guest_invalid state
	e = __snp_lookup_rmpentry(pfn, &level);

	if (!e) {
		printk("==%s: rmpentry not found for pfn 0x%llx\n", __func__,
		       pfn);
		__free_page(page);
		return 0;
	}

	// printk("==%s: found rmpentry for pfn 0x%llx at level %d\n", __func__,
	//        pfn, level);

	if (rmpentry_asid(e) == 0) {
		printk("==%s: rmpentry_asid(e) == 0, just free the page directly\n",
		       __func__);
		__free_page(page);
		return 0;
	}

	// printk("==%s: before rmp_make_shared\n", __func__);
	// dump_rmpentry(pfn);

	ret = rmp_make_shared(pfn, PG_LEVEL_4K);

	// printk("==%s: after rmp_make_shared\n", __func__);
	// dump_rmpentry(pfn);

	if (ret) {
		/*
		 * rmpupdate failed, most likely because the RMP entry still has
		 * immutable=1 (the page is guest-owned and only the guest can
		 * clear that via PVALIDATE).  Do NOT return the page to the host
		 * allocator: its RMP entry still reads assigned=1 / immutable=1 /
		 * asid=<guest>, so any future host reuse of this physical frame
		 * would trigger an RMP violation.  Leave the page quarantined.
		 */
		printk(KERN_ERR
		       "snp_free_guest_invalid_page: rmp_make_shared failed (ret=%d) "
		       "for pfn 0x%llx – page quarantined, NOT returned to allocator\n",
		       ret, pfn);
		/* __free_page(page); -- intentionally omitted: RMP still guest-owned */
		return ret;
	}

	__free_page(page);

	// printk(KERN_INFO "snp_free_guest_invalid_page: Page freed\n");

	return 0;
}
EXPORT_SYMBOL_GPL(snp_free_guest_invalid_page);
