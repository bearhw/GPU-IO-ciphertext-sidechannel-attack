...
#define kvmm_ssdbg_log(fmt, ...) ;printk(fmt, ##__VA_ARGS__);
...
static void kvm_destroy_vm(struct kvm *kvm)
{
	...
	global_sev_step_config.null_cipher_repr_gpa = (uint64_t)(-1);
	global_sev_step_config.null_cipher_repr_spa = (uint64_t)(-1);
	if (global_sev_step_config.null_cipher_repr != NULL) {
		kfree(global_sev_step_config.null_cipher_repr);
		global_sev_step_config.null_cipher_repr = NULL;
	}

	// if (destroy_page_ciphertext_cache(&global_sev_step_config)) {
	// 	printk("KVM_USP_CLOSE_POLL_API: "
	// 	       "failed to destroy page ciphertext cache\n");
	// 	mutex_unlock(&sev_step_config_mutex);
	// 	return -EFAULT;
	// }

	if (destroy_page_null_cipher_repr(&global_sev_step_config)) {
		printk("KVM_USP_CLOSE_POLL_API: "
		       "failed to destroy page null cipher representation\n");
		mutex_unlock(&sev_step_config_mutex);
		// return -EFAULT;
		// return;
	}
			   ...
			   unsigned int ioctl, unsigned long arg)
{
	...
	if (vcpu->kvm->mm != current->mm) {
		...
		/*
		 * swap_pages steals QEMU's vcpu_fd via pidfd_getfd and issues
		 * KVM_RUN from a different process to flush the deferred swap
		 * queued by KVM_SWAP_GUEST_PAGES.  Allow this only when a swap
		 * is actually pending and the ioctl is KVM_RUN; reject everything
		 * else to preserve normal security semantics.
		 */
		if (ioctl == KVM_RUN) {
			bool swap_pending;

			mutex_lock(&sev_step_config_mutex);
			swap_pending = global_sev_step_config.is_swap_pages;
			mutex_unlock(&sev_step_config_mutex);

			if (!swap_pending)
				return -EIO;
			/* fall through: allow KVM_RUN to trigger the swap */
		} else {
			return -EIO;
		}
#else
...
static int kvm_dev_ioctl_create_vm(unsigned long type)
{
	...
	global_sev_step_config.main_vm_pid = task_pid_nr(current);
	// hash_init(global_sev_step_config.ciphertext_pages_cache);
#ifdef ENABLE_OLD_NULL_CIPHER_REPR_SUPPORT
	hash_init(global_sev_step_config.pages_null_cipher_repr);
#endif // ENABLE_OLD_NULL_CIPHER_REPR_SUPPORT
	// spin_lock_init(&global_sev_step_config.ciphertext_pages_cache_lock);
	mutex_unlock(&sev_step_config_mutex);
	printk("setting main_vm to newly started vm\n");
	printk("setting main_vm_pid to %d\n",task_pid_nr(current));
...
void free_kernel_mappings_for_user_eviction_set(addr_list_t* kern,
	...
	kvmm_ssdbg_log("%s:%d all vunmap calls succeeded",__FILE__,__LINE__);
	freeAddrListEntries(kern);
	kvmm_ssdbg_log("%s:%d freeAddrListEntries succeeded",__FILE__,__LINE__);

	//sanity check refcount values before unpin
	for(idx=0; idx < pinned_pages_len; idx++) {
		if(page_ref_count(pinned_pages[idx]) < 0 ) {
			printk("%s:%d [%s] unpinning pfn 0x%lx, refcount before: %d\n",__FILE__,__LINE__,__FUNCTION__,page_to_pfn(pinned_pages[idx]),page_ref_count(pinned_pages[idx]));
		}
	}

	unpin_user_pages(pinned_pages, pinned_pages_len);

	//sanity check refcount values after unpin
	for(idx=0; idx < pinned_pages_len; idx++) {
		if( page_ref_count(pinned_pages[idx]) < 0 ) {
			printk("%s:%d [%s] pfn 0x%lx, Refcount after: %d\n",__FILE__,__LINE__,__FUNCTION__,page_to_pfn(pinned_pages[idx]),page_ref_count(pinned_pages[idx]));
		}

	}

	//printk("%s:%d [%s] end of unpin log",__FILE__,__LINE__,__FUNCTION__);

	kvmm_ssdbg_log("%s:%d unpin_user_pages succeeded",__FILE__,__LINE__);
}
...
static int get_aliased_mapping_for_gpa(uint64_t gpa, uint64_t* hva_alias,
	...
	if (pin_user_pages_remote(global_sev_step_config.main_vm->mm,
		...
		kvmm_ssdbg_log("get_user_pages_remote_unlocked was successfull");
	}
	mmap_read_unlock(global_sev_step_config.main_vm->mm);
	...
	for( idx = 0; idx < user->eviction_sets_len; idx++ ) {
		...
		if( err <= 0 ) {
			printk("%s:%d : get_kernel_mappings_for_user_evictin_set :"
				"get_user_pages_unlocked failed with %d for idx %llu",__FILE__,__LINE__,err,idx);
			//if we fail, here, we successfully pinned idx+1 pages -> unpin them before returning
			unpin_user_pages(pinned_pages, idx);
			return 1;
		}
		pinned_pages[idx] = page;
		//sanity check refcount values
		if( page_ref_count(page) < 0 ) {
			printk("%s:%d [%s] vaddr 0x%llx pinned to 0x%lx. Refcount %d\n",__FILE__,__LINE__,__FUNCTION__,user->eviction_sets[idx] & ~0xfff,page_to_pfn(page),page_ref_count(page));
		}
		...
		if( kernel_mapping == NULL ) {
			...
			free_kernel_mappings_for_user_eviction_set(kern,pinned_pages,idx);
			return 1;
		}
			  ...
			  unsigned int ioctl, unsigned long arg)
{
	...
	/* ── SEV-Step DMA tracking ioctls ──────────────────────── */
	case KVM_DMA_TRACK_ENABLE:
		pr_info("kvm_dev_ioctl: KVM_DMA_TRACK_ENABLE pid=%d comm=%.16s\n",
			current->pid, current->comm);
		r = dma_track_enable();
		break;

	case KVM_DMA_TRACK_DISABLE:
		pr_info("kvm_dev_ioctl: KVM_DMA_TRACK_DISABLE pid=%d\n",
			current->pid);
		r = dma_track_disable();
		break;

	case KVM_DMA_TRACK_DRAIN:
		pr_info("kvm_dev_ioctl: KVM_DMA_TRACK_DRAIN pid=%d\n",
			current->pid);
		r = dma_track_drain(argp);
		break;
	/* ── end SEV-Step DMA tracking ──────────────────────────── */

	/* ── SEV-Step Src-page tracking ioctls ─────────────────── */
	case KVM_SRC_TRACK_ENABLE: {
		struct kvm_src_track_enable_param p;
		if (copy_from_user(&p, argp, sizeof(p)))
			return -EFAULT;
		pr_info("kvm_dev_ioctl: KVM_SRC_TRACK_ENABLE "
			"start_gfn=%llu nr=%llu\n",
			p.start_gfn, p.nr_pages);
		r = src_track_enable(p.start_gfn, p.nr_pages);
		break;
	}
	case KVM_SRC_TRACK_DISABLE:
		pr_info("kvm_dev_ioctl: KVM_SRC_TRACK_DISABLE\n");
		r = src_track_disable();
		break;
	case KVM_SRC_TRACK_DRAIN:
		r = src_track_drain(argp);
		break;
	/* ── end SEV-Step Src-page tracking ─────────────────────── */

	/* ── SEV-Step Dst-page tracking ioctls ─────────────────── */
	case KVM_DST_TRACK_ENABLE: {
		struct kvm_dst_track_enable_param p;
		if (copy_from_user(&p, argp, sizeof(p)))
			return -EFAULT;
		pr_info("kvm_dev_ioctl: KVM_DST_TRACK_ENABLE "
			"start_gfn=%llu nr=%llu\n",
			p.start_gfn, p.nr_pages);
		r = dst_track_enable(p.start_gfn, p.nr_pages);
		break;
	}
	case KVM_DST_TRACK_DISABLE:
		pr_info("kvm_dev_ioctl: KVM_DST_TRACK_DISABLE\n");
		r = dst_track_disable();
		break;
	case KVM_DST_TRACK_DRAIN:
		r = dst_track_drain(argp);
		break;
	/* ── end SEV-Step Dst-page tracking ─────────────────────── */

	/* ── SEV-Step PSC tracking for NV BB detection ─────────── */
	case KVM_PSC_TRACK_ENABLE:
		pr_info("kvm_dev_ioctl: KVM_PSC_TRACK_ENABLE pid=%d\n",
			current->pid);
		r = psc_track_enable();
		break;
	case KVM_PSC_TRACK_DISABLE:
		pr_info("kvm_dev_ioctl: KVM_PSC_TRACK_DISABLE pid=%d\n",
			current->pid);
		r = psc_track_disable();
		break;
	case KVM_PSC_TRACK_DRAIN:
		r = psc_track_drain(argp);
		break;
	/* ── end SEV-Step PSC tracking ─────────────────────────── */

	/* ── SEV-Step NV BB NPT tracking + correlation ─────────── */
	case KVM_NV_BB_START:
		pr_info("kvm_dev_ioctl: KVM_NV_BB_START pid=%d\n",
			current->pid);
		r = nv_bb_start(argp);
		break;
	case KVM_NV_BB_DRAIN:
		r = nv_bb_drain_xfers(argp);
		break;
	case KVM_NV_BB_REARM:
		r = nv_bb_rearm();
		break;
	case KVM_NV_BB_STOP:
		pr_info("kvm_dev_ioctl: KVM_NV_BB_STOP pid=%d\n",
			current->pid);
		r = nv_bb_stop();
		break;
	/* ── end SEV-Step NV BB tracking ───────────────────────── */

	/* ── SEV-Step NV BB single-stepping ────────────────────── */
	case KVM_NV_BB_STEP_ARM:
		pr_info("kvm_dev_ioctl: KVM_NV_BB_STEP_ARM pid=%d\n",
			current->pid);
		r = nv_bb_step_arm(argp);
		break;
	case KVM_NV_BB_STEP_DISARM:
		pr_info("kvm_dev_ioctl: KVM_NV_BB_STEP_DISARM pid=%d\n",
			current->pid);
		r = nv_bb_step_disarm();
		break;
	case KVM_NV_BB_TRIGGERED_STEP:
		pr_info("kvm_dev_ioctl: KVM_NV_BB_TRIGGERED_STEP pid=%d\n",
			current->pid);
		r = nv_bb_triggered_step_arm(argp);
		break;
	/* ── end SEV-Step NV BB stepping ───────────────────────── */
	...
	case KVM_TRACK_PAGE: {
		...
		if (global_sev_step_config.nr_track_events == 0) {
			printk("KVM_TRACK_PAGE: "
			       "There is no existing tracking, "
			       "creating a new one\n");
			global_sev_step_config.nr_track_events = 1;
			global_sev_step_config.is_dump_pre_fault_when_track = param.enable_pre_fault_dump;
			global_sev_step_config.is_dump_post_fault_when_track = param.enable_post_fault_dump;
			global_sev_step_config.is_decrypt_when_track = param.is_decrypt_when_track;
			global_sev_step_config.is_relocate_when_track = param.is_relocate_when_track;
		} else {
			// There is an existing tracking
			if (global_sev_step_config.is_dump_pre_fault_when_track !=
				    param.enable_pre_fault_dump ||
				global_sev_step_config.is_dump_post_fault_when_track !=
				    param.enable_post_fault_dump ||
			    global_sev_step_config.is_decrypt_when_track !=
				    param.is_decrypt_when_track ||
				global_sev_step_config.is_relocate_when_track !=
				    param.is_relocate_when_track) {
				printk("KVM_TRACK_PAGE: "
				       "enable_dump or is_decrypt_when_track is "
				       "different from the existing tracking or "
				       "is_relocate_when_track is different from "
				       "the existing tracking\n");
				mutex_unlock(&sev_step_config_mutex);
				return -EFAULT;
			}
		...
		// Remove the gpa from the untracked list
		if (global_sev_step_config.is_observe_untracked_pages) {
			remove_page_from_untracked_pages_list(
				&global_sev_step_config, param.gpa);
		}
#endif // ENABLE_MONITOR_UNTRACKED_PAGES
		mutex_unlock(&sev_step_config_mutex);

		vcpu = xa_load(&global_sev_step_config.main_vm->vcpu_array, 0);
		if (!__track_single_page(vcpu, param.gpa >> PAGE_SHIFT,
					 param.track_mode, true)) {
	...
	case KVM_UNTRACK_PAGE: {
		track_page_param_t param;
		kvmm_ssdbg_log("KVM_UNTRACK_PAGE: got called\n");
		...
		mutex_lock(&sev_step_config_mutex);
		if (global_sev_step_config.main_vm == NULL) {
			printk("KVM_UNTRACK_PAGE: main_vm is not initialized, aborting!\n");
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		}
		global_sev_step_config.nr_track_events = 0;
		global_sev_step_config.is_dump_pre_fault_when_track = false;
		global_sev_step_config.is_dump_post_fault_when_track = false;
		global_sev_step_config.is_decrypt_when_track = false;
		global_sev_step_config.is_relocate_when_track = false;
		global_sev_step_config.curr_fault_gpa = (uint64_t)-1;
		global_sev_step_config.prev_fault_gpa = (uint64_t)-1;
		mutex_unlock(&sev_step_config_mutex);
		...
		kvmm_ssdbg_log("KVM_UNTRACK_PAGE successfull!\n");
		r = 0;
	} break;
	case KVM_TRACK_ALL_PAGES: {
		...
		// kvmm_ssdbg_log("KVM_TRACK_ALL_PAGES: got called with "
		// 	       "track_mode=%d, enable_dump=%d\n",
		// 	       param.track_mode, param.enable_dump);
		...
		if (param.nr_track_events == 0) {
			printk("KVM_TRACK_ALL_PAGES: nr_track_events should not be 0\n");
			return -EINVAL;
		}

		if (param.is_relocate_when_track &&
		    param.is_decrypt_when_track) {
			printk("KVM_TRACK_ALL_PAGES: is_relocate_when_track and is_decrypt_when_track should not be both true\n");
			return -EINVAL;
		}

#ifdef ENABLE_MONITOR_UNTRACKED_PAGES
		if (param.nr_special_values_when_track > 0 &&
		    param.special_values_when_track == NULL) {
			printk("KVM_TRACK_ALL_PAGES: special_values_when_track should not be NULL\n");
			return -EINVAL;
		}

		if (param.is_send_all_updated_pages && param.nr_track_events > 0) {
			printk("KVM_TRACK_ALL_PAGES: is_send_all_updated_pages should be false when nr_track_events > 0\n");
			return -EINVAL;
		}
#endif // ENABLE_MONITOR_UNTRACKED_PAGES
		...
		if (param.is_relocate_when_track) {
			if (global_sev_step_config.target_page_pfn == (u64)-1 ||
			    global_sev_step_config.target_page_hva == 0 ||
			    global_sev_step_config.target_page_null_ciphertexts ==
				    NULL) {
				printk(KERN_ERR
				       "KVM_TRACK_ALL_PAGES: target_page_pfn is not set "
				       "while is_relocate_when_track is true,"
				       "aborting!\n");
				mutex_unlock(&sev_step_config_mutex);
				return -EFAULT;
			}
...
#ifdef ENABLE_MONITOR_UNTRACKED_PAGES
		if (param.nr_special_values_when_track > 0) {
			global_sev_step_config.special_values_when_track =
				kmalloc(param.nr_special_values_when_track *
						sizeof(uint64_t),
					GFP_KERNEL);
			if (global_sev_step_config.special_values_when_track == NULL) {
				printk("KVM_TRACK_ALL_PAGES: failed to allocate memory for special_values_when_track\n");
				mutex_unlock(&sev_step_config_mutex);
				return -ENOMEM;
			}

			// Copy special values to global_sev_step_config
			if (copy_from_user(global_sev_step_config
						   .special_values_when_track,
					   param.special_values_when_track,
					   param.nr_special_values_when_track *
						   sizeof(uint64_t))) {
				printk("KVM_TRACK_ALL_PAGES: failed to copy special_values_when_track\n");
				kfree(global_sev_step_config.special_values_when_track);
				mutex_unlock(&sev_step_config_mutex);
				return -EINVAL;
			}
			global_sev_step_config.nr_special_values_when_track =
				param.nr_special_values_when_track;
		}
#endif // ENABLE_MONITOR_UNTRACKED_PAGES
		...
		global_sev_step_config.is_decrypt_when_track = param.is_decrypt_when_track;
		global_sev_step_config.is_relocate_when_track = param.is_relocate_when_track;
		global_sev_step_config.nr_track_events = param.nr_track_events;
#ifdef ENABLE_MONITOR_UNTRACKED_PAGES
		global_sev_step_config.is_observe_untracked_pages = param.is_observe_untracked_pages;
		destroy_untracked_pages_list(&global_sev_step_config);
		INIT_LIST_HEAD(&global_sev_step_config.untracked_pages_list);
		global_sev_step_config.is_send_all_updated_pages = param.is_send_all_updated_pages;
#endif // ENABLE_MONITOR_UNTRACKED_PAGES
	...
	case KVM_TRACK_PAGES: {
		u64 *track_gpas;
		struct track_pages_param param;
		u64 nr_tracked_pages;

		if (copy_from_user(&param, argp, sizeof(param))) {
			printk("KVM_TRACK_PAGES: failed to copy args\n");
			return -EINVAL;
		}

		if (param.nr_pages ==
		    0) { printk("KVM_TRACK_PAGES: nr_pages should not be 0\n");
			 return -EINVAL; }

		if (param.track_gpas == NULL) {
			printk("KVM_TRACK_PAGES: pages should not be NULL\n");
			return -EINVAL;
		}
		...
		if (param.track_mode < 0 ||
		    param.track_mode >= KVM_PAGE_TRACK_MAX) {
			printk("KVM_TRACK_PAGES: track_mode %d invalid, must be in range [%d,%d]",
			       param.track_mode, 0, KVM_PAGE_TRACK_MAX);
			return -EFAULT;
		}

		track_gpas = kmalloc(param.nr_pages * sizeof(u64), GFP_KERNEL);
		if (track_gpas == NULL) {
			printk("KVM_TRACK_PAGES: failed to allocate memory for tracked_gpas\n");
			return -ENOMEM;
		}

		if (copy_from_user(track_gpas, param.track_gpas,
				   param.nr_pages * sizeof(u64))) {
			printk("KVM_TRACK_PAGES: failed to copy tracked_gpas\n");
			kfree(track_gpas);
			return -EINVAL;
		}
		...
		mutex_lock(&sev_step_config_mutex);
		if (global_sev_step_config.main_vm == NULL) {
			printk("KVM_TRACK_PAGES: main_vm is not initialized, aborting!\n");
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		}
		...
		nr_tracked_pages = kvm_track_pages(
			xa_load(&global_sev_step_config.main_vm->vcpu_array, 0),
			param.nr_pages, track_gpas, param.track_mode);

		kfree(track_gpas);

		printk("KVM_TRACK_PAGES: tracked %llu pages while %llu pages are requested\n",
		       nr_tracked_pages, param.nr_pages);

		r = 0;
	} break;
	case KVM_TRACK_ALL_PAGES_EXCEPT: {
		struct track_all_pages_except_param param;
		long tracked_pages;

		if (copy_from_user(&param, argp, sizeof(param))) {
			printk("KVM_TRACK_ALL_PAGES_EXCEPT: failed to copy args\n");
			return -EINVAL;
		}

		if (param.nr_except_pages == 0) {
			printk("KVM_TRACK_ALL_PAGES_EXCEPT: nr_except_pages should not be 0\n");
			return -EINVAL;
		}

		if (param.except_gpas == NULL) {
			printk("KVM_TRACK_ALL_PAGES_EXCEPT: except_gpas should not be NULL\n");
			return -EINVAL;
		}

		if (param.track_mode < 0 ||
		    param.track_mode >= KVM_PAGE_TRACK_MAX) {
			printk("KVM_TRACK_ALL_PAGES_EXCEPT: track_mode %d invalid, must be in range [%d,%d]",
			       param.track_mode, 0, KVM_PAGE_TRACK_MAX);
			return -EFAULT;
		}
		...
		mutex_lock(&sev_step_config_mutex);
		if (global_sev_step_config.main_vm == NULL) {
			printk("KVM_TRACK_ALL_PAGES_EXCEPT: main_vm is not initialized, aborting!\n");
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		}
		...
		tracked_pages = kvm_track_all_pages_except(
			xa_load(&global_sev_step_config.main_vm->vcpu_array, 0),
			param.nr_except_pages, param.except_gpas, param.track_mode);

		printk("KVM_TRACK_ALL_PAGES_EXCEPT: tracked %ld pages\n",
		       tracked_pages);
		r = 0;
	} break;
	case KVM_DUMP_GUEST_PAGES: {
		struct kvm_dump_pages_param_t param;
		uint64_t *usp_dump_page_gpas = NULL;
		struct kvm_vcpu *vcpu;
		unsigned nr_dump_pages = 0;
		bool is_decrypt_when_dump = false;
		bool is_relocate_when_dump = false;
		int ret = 0;
		...
		if (copy_from_user(&param, argp, sizeof(param))) {
			printk(KERN_ERR
			       "KVM_DUMP_GUEST_PAGES: failed to copy args\n");
			ret = -EINVAL;
			goto dump_guest_pages_out;
		}

		is_decrypt_when_dump = param.is_decrypt_when_dump;
		is_relocate_when_dump = param.is_relocate_when_dump;
		nr_dump_pages = param.nr_dump_pages;
		usp_dump_page_gpas = param.dump_page_gpas;

		if (is_decrypt_when_dump && is_relocate_when_dump) {
			printk(KERN_ERR
			       "KVM_DUMP_GUEST_PAGES: is_decrypt_when_dump and is_relocate_when_dump should not be both true\n");
			ret = -EINVAL;
			goto dump_guest_pages_out;
		}

		mutex_lock(&sev_step_config_mutex);
		global_sev_step_config.is_dump_page = true;
		global_sev_step_config.nr_dump_pages = nr_dump_pages;
		global_sev_step_config.is_decrypt_when_dump =
			is_decrypt_when_dump;
		global_sev_step_config.is_relocate_when_dump =
			param.is_relocate_when_dump;

		global_sev_step_config.dump_page_gpas =
			kmalloc(nr_dump_pages * sizeof(uint64_t), GFP_KERNEL);
		if (global_sev_step_config.dump_page_gpas == NULL) {
			printk(KERN_ERR
			       "KVM_DUMP_GUEST_PAGES: failed to allocate memory for kern_dump_page_gpas\n");
			ret = -ENOMEM;
			mutex_unlock(&sev_step_config_mutex);
			goto dump_guest_pages_out;
		}

		if (copy_from_user(global_sev_step_config.dump_page_gpas,
				   usp_dump_page_gpas,
				   nr_dump_pages * sizeof(uint64_t))) {
			printk(KERN_ERR
			       "KVM_DUMP_GUEST_PAGES: failed to copy kern_dump_page_gpas\n");
			mutex_unlock(&sev_step_config_mutex);
			ret = -EINVAL;
			goto dump_guest_pages_out;
		}
		...
		{
			// Check the gpas are all page aligned
			size_t i;
			for (i = 0; i < nr_dump_pages; i++) {
				u64 gpa = global_sev_step_config.dump_page_gpas[i];
				if (gpa & (PAGE_SIZE - 1)) {
					printk(KERN_ERR
					       "KVM_DUMP_GUEST_PAGES: gpa %llx is not page aligned\n",
					       usp_dump_page_gpas[i]);
					mutex_unlock(&sev_step_config_mutex);
					ret = -EINVAL;
					goto dump_guest_pages_out;
				}
		...
		if (global_sev_step_config.main_vm == NULL) {
			printk(KERN_ERR
			       "KVM_DUMP_GUEST_PAGES: main_vm is not initialized, aborting!\n");
			mutex_unlock(&sev_step_config_mutex);
			ret = -EFAULT;
			goto dump_guest_pages_out;
		}
...
dump_guest_pages_out:
		if (ret)
			return ret;

		// TODO: see arch/x86/kvm/mmu/mmu.c/page_fault_handle_page_track
		r = 0;
	} break;
	case KVM_DUMP_GUEST_PRIVATE_PAGES: {
		mutex_lock(&sev_step_config_mutex);
		global_sev_step_config.is_dump_private_pages = true;
		mutex_unlock(&sev_step_config_mutex);
		r = 0;
	} break;
	case KVM_GET_NULL_CIPHERTEXTS_ON_TARGET_PAGE: {
		struct kvm_null_ciphertexts_info_param param;
		void *usp_null_ciphertexts_pointer = NULL;
		if (copy_from_user(&param, argp, sizeof(param))) {
			printk("KVM_GET_NULL_CIPHERTEXTS_ON_TARGET_PAGE: failed to copy args\n");
			return -EINVAL;
		}

		usp_null_ciphertexts_pointer =
			(void *)param.usp_null_ciphertexts_ptr;

		if (usp_null_ciphertexts_pointer == NULL) {
			printk("KVM_GET_NULL_CIPHERTEXTS_ON_TARGET_PAGE: usp_null_ciphertexts_ptr should not be NULL\n");
			return -EINVAL;
		}

		mutex_lock(&sev_step_config_mutex);
		if (global_sev_step_config.main_vm == NULL) {
			printk("KVM_GET_NULL_CIPHERTEXTS_ON_TARGET_PAGE: main_vm is not initialized, aborting!\n");
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		}

		if (global_sev_step_config.target_page_null_ciphertexts ==
		    NULL) {
			printk("KVM_GET_NULL_CIPHERTEXTS_ON_TARGET_PAGE: target_page_null_ciphertexts is NULL\n");
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		}
		if (copy_to_user(
			    ...
			    global_sev_step_config.target_page_null_ciphertexts,
			    PAGE_SIZE)) {
			printk("KVM_GET_NULL_CIPHERTEXTS_ON_TARGET_PAGE: failed to copy null ciphertexts\n");
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		}

		mutex_unlock(&sev_step_config_mutex);
		printk("KVM_GET_NULL_CIPHERTEXTS_ON_TARGET_PAGE: success\n");

		r = 0;
	} break;
	case KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: {
		struct kvm_gfn_content_param param;
		void *usp_ct_pointer = NULL;
		u64 gfn_pfn = 0;
		u64 gfn = 0;
		struct kvm *kvm;
		u8 *ciphertext_content = NULL;
		void *target_hva = NULL;
		u64 target_pfn = 0;
		int i = 0;

		// Allocate ciphertext_content first
		ciphertext_content = kmalloc(PAGE_SIZE, GFP_KERNEL);
		if (ciphertext_content == NULL) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: failed to allocate memory for ciphertext_content\n");
			return -ENOMEM;
		}

		if (copy_from_user(&param, argp, sizeof(param))) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: failed to copy args\n");
			kfree(ciphertext_content);
			return -EINVAL;
		}

		usp_ct_pointer = (void *)param.usp_gfn_content_ptr;
		gfn = param.gfn;

		if (usp_ct_pointer == NULL) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: usp_gfn_ciphertexts_ptr should not be NULL\n");
			kfree(ciphertext_content);
			return -EINVAL;
		}
		...
		mutex_lock(&sev_step_config_mutex);
		if (global_sev_step_config.main_vm == NULL) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: main_vm is not initialized, aborting!\n");
			mutex_unlock(&sev_step_config_mutex);
			kfree(ciphertext_content);
			return -EFAULT;
		}
		...
		if (global_sev_step_config.target_page_pfn == (u64)-1) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: target_page_pfn is not set\n");
			mutex_unlock(&sev_step_config_mutex);
			kfree(ciphertext_content);
			return -EFAULT;
		}

		target_hva = (void *)global_sev_step_config.target_page_hva;
		target_pfn = global_sev_step_config.target_page_pfn;
		if (target_hva == NULL || target_pfn == (u64)-1) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: target_hva or target_pfn is not set\n");
			mutex_unlock(&sev_step_config_mutex);
			kfree(ciphertext_content);
			return -EFAULT;
		}

		global_sev_step_config.gfn_to_translate = gfn;
		global_sev_step_config.translated_pfn_of_gfn = (u64)-1;
		mutex_unlock(&sev_step_config_mutex);

		// Polling for gfn_pfn to be non -1
		for (i = 0; i < 10000; i++) {
			mutex_lock(&sev_step_config_mutex);
			gfn_pfn = global_sev_step_config.translated_pfn_of_gfn;
			if (gfn_pfn != (u64)-1) {
				mutex_unlock(&sev_step_config_mutex);
				break;
			}
			...
			cond_resched();
			mutex_unlock(&sev_step_config_mutex);
			// Sleep for several ms to avoid busy waiting
			usleep_range(1000, 2000);
		}

		if (gfn_pfn == (u64)-1) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: gfn_pfn is still -1 after polling\n");
			kfree(ciphertext_content);
			// Reset the gfn_to_translate and translated_pfn_of_gfn
			mutex_lock(&sev_step_config_mutex);
			global_sev_step_config.gfn_to_translate = (u64)-1;
			global_sev_step_config.translated_pfn_of_gfn = (u64)-1;
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		} else {
			...
			global_sev_step_config.translated_pfn_of_gfn = (u64)-1;
			mutex_unlock(&sev_step_config_mutex);
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: gfn_pfn is %llx\n", gfn_pfn);
		}

		if (!pfn_valid(gfn_pfn)) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: gfn %llx is not valid (gfn_pfn=%llx)\n",
			       gfn, gfn_pfn);
			kfree(ciphertext_content);
			return -EFAULT;
		}

		if (get_gfn_ciphertext_at_pfn(kvm, gfn, gfn_pfn, target_hva,
					       target_pfn, ciphertext_content,
					       true)) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: get_gfn_ciphertexts_at_pfn failed\n");
			kfree(ciphertext_content);
			return -EFAULT;
		}

		if (copy_to_user(usp_ct_pointer, ciphertext_content, PAGE_SIZE)) {
			printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: failed to copy ciphertext_content\n");
			kfree(ciphertext_content);
			return -EFAULT;
		}
		kfree(ciphertext_content);
		printk("KVM_GET_GFN_CIPHERTEXTS_ON_TARGET_PAGE: success\n");
		r = 0;
	} break;
	case KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: {
		struct kvm_gfn_content_param param;
		void *usp_pt_pointer = NULL;
		u64 gfn_pfn = (u64)-1;
		u64 gfn = (u64)-1;
		struct kvm *kvm;
		u8 *plaintext_content = NULL;
		int ret = 0;
		int i = 0;

		plaintext_content = kmalloc(PAGE_SIZE, GFP_KERNEL);
		if (plaintext_content == NULL) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: failed to allocate memory for plaintext_content\n");
			return -ENOMEM;
		}

		if (copy_from_user(&param, argp, sizeof(param))) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: failed to copy args\n");
			kfree(plaintext_content);
			return -EINVAL;
		}

		usp_pt_pointer = (void *)param.usp_gfn_content_ptr;
		gfn = param.gfn;

		if (usp_pt_pointer == NULL) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: usp_gfn_plaintexts_ptr should not be NULL\n");
			kfree(plaintext_content);
			return -EINVAL;
		}
		...
		mutex_lock(&sev_step_config_mutex);
		if (global_sev_step_config.main_vm == NULL) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: main_vm is not initialized, aborting!\n");
			mutex_unlock(&sev_step_config_mutex);
			kfree(plaintext_content);
			return -EFAULT;
		}
		...
		if (!pfn_valid(gfn_pfn)) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: gfn %llx is not valid (gfn_pfn=%llx)\n",
			       gfn, gfn_pfn);
			kfree(plaintext_content);
			return -EFAULT;
		}

		ret = get_gfn_plaintext_by_decrypt_in_debug_mode(
			kvm, gfn_pfn, plaintext_content);

		if (ret) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: get_gfn_plaintext_by_decrypt_in_debug_mode failed\n");
			kfree(plaintext_content);
			return -EFAULT;
		}

		if (copy_to_user(usp_pt_pointer, plaintext_content, PAGE_SIZE)) {
			printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: failed to copy plaintext_content\n");
			kfree(plaintext_content);
			return -EFAULT;
		}
		kfree(plaintext_content);
		printk("KVM_GET_GFN_PLAINTEXTS_IN_DEBUG_MODE: success\n");
		r = 0;
	} break;
#ifdef ENABLE_OLD_SWAP_PAGES_SUPPORT
	case KVM_SWAP_GUEST_PAGES: {
		struct kvm_swap_pages_param_t param;

		kvmm_ssdbg_log("KVM_SWAP_GUEST_PAGES: got called\n");

		mutex_lock(&sev_step_config_mutex);
		if (global_sev_step_config.main_vm == NULL) {
			mutex_unlock(&sev_step_config_mutex);
			printk("KVM_SWAP_GUEST_PAGES: main_vm is not initialized, aborting!\n");
			return -EFAULT;
		}
		...
		if (copy_from_user(&param, argp, sizeof(param))) {
			mutex_unlock(&sev_step_config_mutex);
			printk("KVM_SWAP_GUEST_PAGES: failed to copy args\n");
			return -EINVAL;
		}

		if (param.nr_swap_times < 1) {
			mutex_unlock(&sev_step_config_mutex);
			printk("KVM_SWAP_GUEST_PAGES: nr_swap_times should be greater than 1\n");
			return -EINVAL;
		}

		// If swap_times > 1, at least one of nr_gfn1_new_assignments and nr_gfn2_new_assignments should be greater than 0
		if (param.nr_swap_times > 1 &&
		    param.nr_gfn1_new_assignments == 0 &&
		    param.nr_gfn2_new_assignments == 0) {
			mutex_unlock(&sev_step_config_mutex);
			printk("KVM_SWAP_GUEST_PAGES: nr_gfn1_new_assignments and nr_gfn2_new_assignments should be greater than 0\n");
			return -EINVAL;
		}

		kvmm_ssdbg_log(
			"KVM_SWAP_GUEST_PAGES: gfn1 0x%llx, gfn2 0x%llx, is_swap_back %d, nr_swap_times %d, nr_gfn1_new_assignments %d, nr_gfn2_new_assignments %d\n",
			param.gfn1, param.gfn2, param.is_swap_back,
			param.nr_swap_times, param.nr_gfn1_new_assignments,
			param.nr_gfn2_new_assignments);

		global_sev_step_config.is_swap_pages = true;
		global_sev_step_config.swap_done = false;
		global_sev_step_config.is_swap_back = param.is_swap_back;
		global_sev_step_config.nr_swap_times = param.nr_swap_times;
		global_sev_step_config.nr_gfn1_new_assignments = param.nr_gfn1_new_assignments;
		global_sev_step_config.nr_gfn2_new_assignments = param.nr_gfn2_new_assignments;

		global_sev_step_config.swap_gfn1 = param.gfn1;
		global_sev_step_config.swap_gfn2 = param.gfn2;

		mutex_unlock(&sev_step_config_mutex);

		/* Kick vcpu0 so QEMU's current KVM_RUN exits the guest and
		 * re-enters, executing the swap in svm_vcpu_run() under the
		 * correct (QEMU) process context. */
		{
			struct kvm_vcpu *vcpu0 =
				kvm_get_vcpu(global_sev_step_config.main_vm, 0);
			if (vcpu0)
				kvm_vcpu_kick(vcpu0);
		}

		printk("KVM_SWAP_GUEST_PAGES: ioctl success, sev_step_config_mutex unlocked, vcpu0 kicked\n");

		r = 0;
	} break;

	case KVM_USP_SWAP_STATUS: {
		uint32_t done;

		mutex_lock(&sev_step_config_mutex);
		done = (uint32_t)global_sev_step_config.swap_done; /* 0=pending, 1=ok, 0xFFFFFFFF=err */
		mutex_unlock(&sev_step_config_mutex);

		if (put_user(done, (uint32_t __user *)argp))
			return -EFAULT;
		r = 0;
	} break;

	case KVM_READ_PAGE_DUMP: {
		mutex_lock(&sev_step_config_mutex);
		if (!global_sev_step_config.page_dump_valid) {
			mutex_unlock(&sev_step_config_mutex);
			return -EAGAIN;
		}
		if (copy_to_user((void __user *)argp,
				 global_sev_step_config.page_dump_buf,
				 PAGE_SIZE)) {
			mutex_unlock(&sev_step_config_mutex);
			return -EFAULT;
		}
		global_sev_step_config.page_dump_valid = false;
		mutex_unlock(&sev_step_config_mutex);
		r = 0;
	} break;
#endif // ENABLE_OLD_SWAP_PAGES_SUPPORT
#ifdef ENABLE_OLD_NULL_CIPHER_REPR_SUPPORT
	case KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: {
		struct kvm_page_ciphertext_param_t param;
		uint8_t *ciphertext_content = NULL;
		int ret = 0;

		kvmm_ssdbg_log("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: got called\n");

		if (copy_from_user(&param, argp, sizeof(param))) {
			printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: failed to copy args\n");
			return -EINVAL;
		}

		if (param.gpa == (uint64_t)-1 || param.ciphertext_content == NULL) {
			printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: invalid arguments\n");
			return -EINVAL;
		}
		...
		if (ciphertext_content == NULL) {
			printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: failed to allocate memory for ciphertext_content\n");
			return -ENOMEM;
		}

		if (copy_from_user(ciphertext_content, param.ciphertext_content, PAGE_SIZE)) {
			printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: failed to copy ciphertext_content\n");
			kfree(ciphertext_content);
			return -EINVAL;
		}
		...
		if (global_sev_step_config.main_vm == NULL) {
			mutex_unlock(&sev_step_config_mutex);
			printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: main_vm is not initialized, aborting!\n");
			kfree(ciphertext_content);
			return -EFAULT;
		}
		...
		// Try to remove the page from the null page list if it is there
		if (is_page_in_null_page_list(&global_sev_step_config,
					      param.gpa)) {
			ret = remove_page_from_null_page_list(
				&global_sev_step_config, param.gpa);
			if (ret) {
				printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: failed to remove page from null page list: gpa 0x%llx\n",
				       param.gpa);
				mutex_unlock(&sev_step_config_mutex);
				kfree(ciphertext_content);
				return ret;
			...
			printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: page is not in null page list\n");
		}

		// if (find_page_null_cipher_repr_at_spa(&global_sev_step_config,
		// 				      param.gpa)) {
		// 	ret = remove_page_null_cipher_repr(
		// 		&global_sev_step_config, param.gpa);
		// 	if (ret) {
		// 		printk("KVM_SET_CIPHERTEXT_PAGE_GROUND_TRUTH: failed to remove page from null cipher repr: gpa 0x%llx\n",
		// 		       param.gpa);
		// 		mutex_unlock(&sev_step_config_mutex);
		// 		kfree(ciphertext_content);
		// 		return ret;
		...
		// Set the new value after removing the old one
		global_sev_step_config.null_cipher_repr_gpa = param.gpa;
		global_sev_step_config.null_cipher_repr_spa = (u64)(-1); // GPA->SPA translation is not done yet
		global_sev_step_config.null_cipher_repr = ciphertext_content;

		ret = insert_page_to_null_page_list(&global_sev_step_config,
						 param.gpa);
		if (ret) {
			mutex_unlock(&sev_step_config_mutex);
			kfree(ciphertext_content);
	...
	case KVM_UNTRACK_ALL_PAGES: {
		...
		kvmm_ssdbg_log("KVM_UNTRACK_ALL_PAGES: got called\n");
...
#ifdef ENABLE_MONITOR_UNTRACKED_PAGES
		global_sev_step_config.is_observe_untracked_pages = false;
		destroy_untracked_pages_list(&global_sev_step_config);
		INIT_LIST_HEAD(&global_sev_step_config.untracked_pages_list);
		global_sev_step_config.is_send_all_updated_pages = false;
		global_sev_step_config.nr_special_values_when_track = 0;
		if (global_sev_step_config.special_values_when_track != NULL) {
			kfree(global_sev_step_config.special_values_when_track);
		...
		kvmm_ssdbg_log(
			"KVM_UNTRACK_ALL_PAGES: untracked %lu pages, track_mode %d\n",
			untrack_count, param.track_mode);

		r = 0;
	} break;
	case KVM_USP_INIT_POLL_API: {
		usp_init_poll_api_t param;
		kvmm_ssdbg_log("KVM_USP_INIT_POLL_API: got called\n");
...
#ifdef ENABLE_OLD_NULL_CIPHER_REPR_SUPPORT
			// Init the null_page_list
			hash_init(global_sev_step_config.null_page_list);
#endif // ENABLE_OLD_NULL_CIPHER_REPR_SUPPORT
			...
			// destroy_untracked_pages_list(&global_sev_step_config);
			INIT_LIST_HEAD(
				&global_sev_step_config.untracked_pages_list);
#endif // ENABLE_MONITOR_UNTRACKED_PAGES
			...
			kvmm_ssdbg_log("KVM_USP_INIT_POLL_API: success\n");
			r = 0;
	} break;
	case KVM_USP_CLOSE_POLL_API: {
		...
		void* kernel_mapping;
		kvmm_ssdbg_log("KVM_USP_CLOSE_POLL_API: got called\n");
			if( uspt_ctx == NULL ) {
				printk("KVM_USP_CLOSE_POLL_API: ctx already null");
				return -EINVAL;
			}

		printk("KVM_USP_CLOSE_POLL_API: context was not null\n");
    		...
    		if( pinned_pages != NULL ) {
				...
				printk("KVM_USP_CLOSE_POLL_API: pinned_pages == NULL, this should not happen\n");
			}
			...
			// Resetting dump page related settings
			global_sev_step_config.is_dump_page = false;
			if (global_sev_step_config.dump_page_gpas != NULL) {
				kfree(global_sev_step_config.dump_page_gpas);
				global_sev_step_config.dump_page_gpas = NULL;
			}
			global_sev_step_config.nr_dump_pages = 0;
			global_sev_step_config.is_decrypt_when_dump = false;
			global_sev_step_config.is_relocate_when_dump = false;
...
#ifdef ENABLE_OLD_SWAP_PAGES_SUPPORT
			global_sev_step_config.is_swap_pages = false;
			global_sev_step_config.is_swap_back = false;
			global_sev_step_config.nr_gfn1_new_assignments = 0,
			global_sev_step_config.nr_gfn2_new_assignments = 0,
			global_sev_step_config.swap_gfn1 = 0;
			global_sev_step_config.swap_gfn2 = 0;
			global_sev_step_config.nr_swap_times = 0;
#endif // ENABLE_OLD_SWAP_PAGES_SUPPORT
			...
			// Destroy the null page list
			if (destroy_null_page_list(&global_sev_step_config)) {
				printk("KVM_USP_CLOSE_POLL_API: "
				       "failed to destroy null page list\n");
				mutex_unlock(&sev_step_config_mutex);
				return -EFAULT;
			}
#endif // ENABLE_OLD_NULL_CIPHER_REPR_SUPPORT
			...
			global_sev_step_config.is_send_all_updated_pages = false;
			global_sev_step_config.nr_special_values_when_track = 0;
			if (global_sev_step_config.special_values_when_track) {
				kfree(global_sev_step_config
					      .special_values_when_track);
			...
			global_sev_step_config.is_dump_private_pages = false;

			mutex_unlock(&sev_step_config_mutex);


			kvmm_ssdbg_log("KVM_USP_CLOSE_POLL_API: success\n");
			r = 0;
	} break;
	case KVM_SEV_STEP_ENABLE: {
		...
		kvmm_ssdbg_log("KVM_SEV_STEP_ENABLE: got called\n");
		...
		global_sev_step_config.do_tlb_flush_before_each_step = param.do_tlb_flush_before_each_step;

		mutex_unlock(&sev_step_config_mutex);

		kvmm_ssdbg_log("KVM_SEV_STEP_ENABLE: success\n");
		r = 0;
	} break;
	case KVM_SEV_STEP_DISABLE: {
		kvmm_ssdbg_log("KVM_SEV_STEP_DISABLE: got called\n");
		mutex_lock(&sev_step_config_mutex);
		switch (global_sev_step_config.single_stepping_status) {
			case SEV_STEP_STEPPING_STATUS_DISABLED:
		...
		kvmm_ssdbg_log("KVM_SEV_STEP_DISABLE: success\n");
		r = 0;
	} break;
	case KVM_SEV_STEP_INJECT_NMI: {
		kvmm_ssdbg_log("KVM_SEV_STEP_INJECT_NMI: got called\n");
	...
	case KVM_SEV_STEP_BUILD_ALIAS_EVS: {
		...
		kvmm_ssdbg_log("KVM_SEV_STEP_BUILD_ALIAS_EVS: got called\n");
		...
		kvmm_ssdbg_log("%s:%d param.attack_targets = 0x%llx, param.attack_targets_len = %llu\n",
		...
		for( attack_target_idx = 0; attack_target_idx < param.attack_targets_len && !error_bulding_ev;
			...
			kvmm_ssdbg_log("%s:%d calling get_aliased_mapping_for_gpa\n",__FILE__,__LINE__);
			...
			kvmm_ssdbg_log("%s:%d aliasing_hva is 0x%llx\n",__FILE__,__LINE__, aliasing_hva);
			...
			for( j = 0; j < cur_lut->table_bytes/64 && !error_bulding_ev ; j++ ) {
				...
				kvmm_ssdbg_log("adding 0x%llx to ev\n",ev_entry);
				insert_end(cur_ev, ev_entry);
				kvmm_ssdbg_log("testing access\n");
				*((volatile uint8_t*)ev_entry);
			}
		}
		kvmm_ssdbg_log("%s:%d main loop done\n",__FILE__,__LINE__);
		...
		if( error_bulding_ev ) {
			...
			kvmm_ssdbg_log("success bulding alias ev!\n");
			for( attack_target_idx = 0; attack_target_idx < param.attack_targets_len;
				attack_target_idx++) {
					kvmm_ssdbg_log("ev %u addr list first addr in list 0x%llx",
						attack_target_idx,
	...
	case KVM_SEV_STEP_BUILD_EVS: {
		...
		bool error_bulding_ev;
		int attack_target_idx;
		kvmm_ssdbg_log("KVM_SEV_STEP_BUILD_EVS: got called\n");
		...
		for( attack_target_idx = 0; attack_target_idx < param.attack_targets_len && !error_bulding_ev;
			...
			kvmm_ssdbg_log("KVM_SEV_STEP_BUILD_EVS: building eviction set for table idx %d,"
			...
			for( j = 0; j < cur_lut->table_bytes/64 && !error_bulding_ev ; j++ ) {
				...
				kvmm_ssdbg_log("KVM_SEV_STEP_BUILD_EVS: calling kernel_get_colliding_vaddr for 0x%llx\n",victim_addr);
				if( kernel_get_colliding_vaddr(victim_addr,&colliding_vaddr)) {
					insert_end(cur_ev,colliding_vaddr);
					kvmm_ssdbg_log("success\n");
				} else {
	...
	case KVM_SEV_STEP_FREE_EVS: {
		...
		kvmm_ssdbg_log("KVM_SEV_STEP_FREE_EVS: got called");
		...
		case SEV_STEP_EV_TYPE_L1D_KERN_ONLY_ALIASING: {
			...
			kvmm_ssdbg_log("%s:%dd KVM_SEV_STEP_FREE_EVS: Going to kfree config->lookup_tables\n",__FILE__,__LINE__);
			kfree(config->lookup_tables);

			kvmm_ssdbg_log("%s:%dd KVM_SEV_STEP_FREE_EVS: Going to upin pages\n",__FILE__,__LINE__);
			...
			kvmm_ssdbg_log("%s:%dd KVM_SEV_STEP_FREE_EVS: Going to kfree config->eviction_sets\n",__FILE__,__LINE__);
			kfree(config->eviction_sets);

			kvmm_ssdbg_log("%s:%dd KVM_SEV_STEP_FREE_EVS: Going to kfree config\n",__FILE__,__LINE__);
			kfree(config);
		}
		break;
		case SEV_STEP_EV_TYPE_FROM_USER: {
			...
			break;
		default:
			kvmm_ssdbg_log("%s:%dd undefined config->type value %d\n",__FILE__,__LINE__,config->type);
			break;
		}
	...
	case KVM_SET_STEP_IMPORT_USER_EVS: {
		...
		kvmm_ssdbg_log("KVM_SET_STEP_IMPORT_USER_EVS: got called\n");
		...
		kvmm_ssdbg_log("%s:%d KVM_SET_STEP_IMPORT_USER_EVS: Got %llu lut and %llu eviction sets with %llu entries each\n",
		...
		if( !global_sev_step_config.state_save_values_valid ) {
			kvmm_ssdbg_log("%s:%d KVM_SET_STEP_IMPORT_USER_EVS: omitting cross check with save"
			...
			kvmm_ssdbg_log("checked %llu pages of kernel_eviction_sets[%llu] for colissions with state save\n",
	...
	case KVM_SEV_STEP_DO_CACHE_ATTACK_NEXT_STEP: {
		...
		kvmm_ssdbg_log("KVM_SEV_STEP_DO_CACHE_ATTACK_NEXT_STEP: got called\n");
	...
	case KVM_SEV_STEP_GPA_TO_HPA: {
		...
		//bool async = false;
		//bool writeable;
		kvmm_ssdbg_log("KVM_SEV_STEP_GPA_TO_HPA: got called\n");
		...
		mmap_read_lock(global_sev_step_config.main_vm->mm);
		locked = 1;
		//TODO: as we do not touch the data here, get_user_pages_remote should actualyl be sufficient
		if (pin_user_pages_remote(global_sev_step_config.main_vm->mm,
		...
		pfn = page_to_pfn(page);
		kvmm_ssdbg_log("pfn is 0x%llx\n",pfn);
		param.out_hpa = (pfn << 12 ) | offset;
		//sanity check refcount value
		if(page_ref_count(page) < 0 ) {
			printk("%s:%d [%s] unpinning pfn 0x%llx. Refcount before: %d\n",__FILE__,__LINE__,__FUNCTION__,pfn,page_ref_count(page));	
		}
		...
		//sanity check refcount value
		if( page_ref_count(page) < 0 ) {
			printk("%s:%d [%s] pfn 0x%llx Refcount after: %d\n",__FILE__,__LINE__,__FUNCTION__,pfn, page_ref_count(page));
		}
		  ...
		  struct module *module)
{
	...
	r = jit_npt_init();
	if (r) {
		pr_warn("kvm: jit_npt_init() failed (%d), "
			"JIT-NPT trapping unavailable\n", r);
		/* non-fatal – KVM continues without JIT-NPT */
	}
