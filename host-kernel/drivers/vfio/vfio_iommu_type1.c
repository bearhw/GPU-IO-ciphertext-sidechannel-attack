...
/*
 * SEV-Step DMA tracking hooks.
 *
 * Defined here (built into vmlinux) and EXPORT_SYMBOL'd so that
 * arch/x86/kvm/sev-step/dma_track.c (kvm.ko) can write them.
 * They are NULL until the userspace tool issues KVM_DMA_TRACK_ENABLE.
 *
 * sev_step_dma_map_hook   – called on successful VFIO MAP_DMA
 * sev_step_dma_unmap_hook – called at the start of vfio_remove_dma()
 */
void (*sev_step_dma_map_hook)(dma_addr_t iova,
			      size_t size, int prot) = NULL;
EXPORT_SYMBOL(sev_step_dma_map_hook);

void (*sev_step_dma_unmap_hook)(dma_addr_t iova,
				size_t size, int prot) = NULL;
EXPORT_SYMBOL(sev_step_dma_unmap_hook);
...
{
	/*
	 * SEV-Step DMA tracking: notify KVM that this window is closing.
	 * Call before unmap so the hook still sees valid dma fields.
	 */
	{
		void (*unmap_hook)(dma_addr_t, size_t, int) =
			READ_ONCE(sev_step_dma_unmap_hook);
		if (unmap_hook) {
			pr_info("vfio_iommu_type1: UNMAP_DMA hook "
				"iova=0x%llx size=0x%zx prot=0x%x "
				"pid=%d comm=%.16s\n",
				(u64)dma->iova, dma->size, dma->prot,
				current->pid, current->comm);
			unmap_hook(dma->iova, dma->size, dma->prot);
		}
	}
			    ...
			    size_t map_size)
{
	...
	if (ret) {
		...
		/*
		 * SEV-Step DMA tracking: notify KVM that a new DMA window
		 * is live.  The hook is NULL unless KVM_DMA_TRACK_ENABLE has
		 * been issued.  dma->iova == GPA for QEMU identity mappings.
		 */
		void (*map_hook)(dma_addr_t, size_t, int) =
			READ_ONCE(sev_step_dma_map_hook);
		if (map_hook) {
			pr_info("vfio_iommu_type1: MAP_DMA hook iova=0x%llx "
				"size=0x%zx prot=0x%x pages=%zu "
				"pid=%d comm=%.16s\n",
				(u64)dma->iova, dma->size, dma->prot,
				dma->size >> PAGE_SHIFT,
				current->pid, current->comm);
			map_hook(dma->iova, dma->size, dma->prot);
		}
	}
