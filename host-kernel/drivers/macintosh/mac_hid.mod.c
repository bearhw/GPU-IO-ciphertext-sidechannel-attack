#include <linux/module.h>
#define INCLUDE_VERMAGIC
#include <linux/build-salt.h>
#include <linux/elfnote-lto.h>
#include <linux/export-internal.h>
#include <linux/vermagic.h>
#include <linux/compiler.h>

BUILD_SALT;
BUILD_LTO_INFO;

MODULE_INFO(vermagic, VERMAGIC_STRING);
MODULE_INFO(name, KBUILD_MODNAME);

__visible struct module __this_module
__section(".gnu.linkonce.this_module") = {
	.name = KBUILD_MODNAME,
	.init = init_module,
#ifdef CONFIG_MODULE_UNLOAD
	.exit = cleanup_module,
#endif
	.arch = MODULE_ARCH_INIT,
};

MODULE_INFO(intree, "Y");

#ifdef CONFIG_RETPOLINE
MODULE_INFO(retpoline, "Y");
#endif


static const struct modversion_info ____versions[]
__used __section("__versions") = {
	{ 0xbdfb6dbb, "__fentry__" },
	{ 0x6886029d, "input_close_device" },
	{ 0x1eee0d0a, "input_unregister_handle" },
	{ 0x37a0cba, "kfree" },
	{ 0xc2261e62, "input_event" },
	{ 0xa3563486, "kmalloc_caches" },
	{ 0x226c1d3a, "kmem_cache_alloc_trace" },
	{ 0xeefe3ef6, "input_register_handle" },
	{ 0x90682d63, "input_open_device" },
	{ 0x92997ed8, "_printk" },
	{ 0x6d16c104, "mutex_lock_killable" },
	{ 0xa8181adf, "proc_dointvec" },
	{ 0x3213f038, "mutex_unlock" },
	{ 0x53bad14e, "input_unregister_handler" },
	{ 0x84ddca63, "input_unregister_device" },
	{ 0x3da716bf, "input_allocate_device" },
	{ 0xaddf64b0, "input_register_device" },
	{ 0x7799f174, "input_register_handler" },
	{ 0xe42878c7, "input_free_device" },
	{ 0x9cd91791, "register_sysctl" },
	{ 0xd691c6a9, "unregister_sysctl_table" },
	{ 0x817bf968, "module_layout" },
};

MODULE_INFO(depends, "");

MODULE_ALIAS("input:b*v*p*e*-e*1,*k*r*a*m*l*s*f*w*");

MODULE_INFO(srcversion, "C7AA10846748E15FE8788C7");
