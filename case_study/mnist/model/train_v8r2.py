"""v8r2 Stage 1: Real-Data UNet+GAN Training (extracted from UNet_training_v2.ipynb)"""
import torch
...
CHUNK_BYTES = 16
PAGE_SIZE   = 4096
IMAGE_SIZE  = 3136   # 28×28×4 bytes (float32)
...
class RealListDataset(Dataset):
    def __init__(self, list_dir, images_db, index_filter=None):
        ...
        print(f"  RealListDataset: {len(self.items)} files loaded")
...
print(f"  v5.json matched samples  : {len(matched)}")
...
history = {'d_loss': [], 'g_loss': [], 'val_mse': [], 'val_ssim': [], 'val_comb': []}
...
print(f"\nUNetGeneratorR | Device: {device}")
print(f"  json_train : {len(json_train_ds)} (matched, augment_sparsity)")
print(f"  real_ds    : {len(real_ds)} (.out_v5.list, real, val excluded)")
print(f"  train total: {len(train_ds)}")
print(f"  val        : {len(val_ds)} (matched, fixed sparsity)")
...
for epoch in range(1, EPOCHS + 1):
    ...
    if v_comb < best_val:
        best_val = v_comb
        torch.save(G.state_dict(), './unet_gan_generator_v8r2.pth')
        tag = " ← saved"
    ...
    history['val_comb'].append(v_comb)

    print(f"[{epoch:3d}/{EPOCHS}]  D:{np.mean(d_losses):.4f}  G:{np.mean(g_losses):.4f}"
          f"  mse:{v_mse:.5f}  SSIM:{v_ssim:.4f}{tag}", flush=True)

print(f"\nDone → unet_gan_generator_v8r2.pth  (best combined: {best_val:.5f})")

# ── Save PDF: training curves ──────────────────────────────────────────────────
epochs_x = list(range(1, len(history['d_loss']) + 1))
fig, axes = plt.subplots(2, 2, figsize=(12, 8))
fig.suptitle('v8r2 Training Curves', fontsize=14)

axes[0, 0].plot(epochs_x, history['d_loss'], label='D loss')
axes[0, 0].set_title('Discriminator Loss'); axes[0, 0].set_xlabel('Epoch'); axes[0, 0].legend()

axes[0, 1].plot(epochs_x, history['g_loss'], label='G loss', color='orange')
axes[0, 1].set_title('Generator Loss'); axes[0, 1].set_xlabel('Epoch'); axes[0, 1].legend()

axes[1, 0].plot(epochs_x, history['val_mse'], label='val MSE', color='red')
axes[1, 0].set_title('Val MSE'); axes[1, 0].set_xlabel('Epoch'); axes[1, 0].legend()

axes[1, 1].plot(epochs_x, history['val_ssim'], label='val SSIM', color='green')
axes[1, 1].set_title('Val SSIM'); axes[1, 1].set_xlabel('Epoch'); axes[1, 1].legend()

plt.tight_layout()
plt.savefig('./train_v8r2_curves.pdf', bbox_inches='tight')
plt.close()
print("Saved → train_v8r2_curves.pdf")
