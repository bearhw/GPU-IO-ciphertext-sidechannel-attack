"""v10: v6r encoder (frozen) + V10Decoder
Training data: .out_v5.list (real sparsity) + json matched target images
Reuses v6r's offset-invariant embedding as-is, trains decoder only
"""
import torch
...
class SparsityEncoder(nn.Module):
    """v6r encoder, frozen during training"""
    def __init__(self):
        super().__init__()
...
class V10Decoder(nn.Module):
    ...
    def forward(self, z):           # z: (B, 128)
        ...
        return self.net(x).squeeze(1)   # (B, 28, 28)
...
class V10Dataset(Dataset):
    def __init__(self, items, augment=True):
        self.items   = items    # list of (real_sp: list[int], img: np.ndarray)
...
enc.load_state_dict(torch.load('./sparsity_encoder_v6r.pth', map_location=device))
enc.eval()
...
sch_Dec = optim.lr_scheduler.CosineAnnealingLR(opt_Dec, T_max=200)
...
print(f"v10 | Device: {device} | Train: {len(train_items)} | Val: {len(val_items)}")
print(f"Enc params (frozen): {sum(p.numel() for p in enc.parameters()):,}")
print(f"Dec params:          {sum(p.numel() for p in Dec.parameters()):,}")
...
for epoch in range(EPOCHS):
    ...
    if v_comb < best_val:
        best_val = v_comb
        torch.save(Dec.state_dict(), './v10_decoder.pth')
        tag = " ← saved"

    print(f"Epoch [{epoch+1:3d}/{EPOCHS}]  D: {np.mean(d_losses):.4f}  "
          f"G: {np.mean(g_losses):.4f}  val_mse: {v_mse:.6f}  "
          f"val_SSIM: {1-v_ssim:.4f}{tag}", flush=True)

print(f"\nDone. Best combined val loss: {best_val:.6f} → v10_decoder.pth")
...
axes[1].set_xlabel('Epoch'); axes[1].set_title('Validation SSIM')
axes[1].legend(); axes[1].grid(True)
plt.tight_layout()
plt.savefig('./train_v10_curves.pdf')
print("Curve saved → train_v10_curves.pdf")
