"""v9r: UNet+PatchGAN, Sparsity Map Only, Real-Data Training
Training data: only .out_v5.list index-matched samples from UNet_training_v3.json (~2,515)
"""
import torch
...
class MatchedJsonDataset(Dataset):
    """
    augment=True  (train): random offset via augment_sparsity on each call
    augment=False (val)  : use stored input_sparsity_seq as-is
    """
...
print(f"v9r | Device: {device} | Train: {n_train} | Val: {n_val}")
...
for epoch in range(EPOCHS):
    ...
    if v_comb < best_val:
        best_val = v_comb
        torch.save(G.state_dict(), './unet_gan_generator_v9r.pth')
        tag = " ← saved"
...
print(f"\nDone. Best: {best_val:.6f} → unet_gan_generator_v9r.pth")
...
plt.tight_layout()
plt.savefig('./train_v9r_curves.pdf')
print("Curve saved → train_v9r_curves.pdf")
