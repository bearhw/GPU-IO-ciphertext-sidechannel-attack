...
class SparsityEncoder(nn.Module):
    """v6r_new encoder, frozen during training"""
    def __init__(self):
        super().__init__()
...
print(f'Matched items: {len(items)}', flush=True)
...
EPOCHS    = 200
best_val  = float('inf')
MODEL_OUT = './v10_decoder_new.pth'

print(f"v10_new | Device: {device} | Train: {len(train_items)} | Val: {len(val_items)}", flush=True)
print(f"Enc params (frozen): {sum(p.numel() for p in enc.parameters()):,}", flush=True)
print(f"Dec params:          {sum(p.numel() for p in Dec.parameters()):,}", flush=True)
print(f"D   params:          {sum(p.numel() for p in D.parameters()):,}", flush=True)
print("-" * 70)
...
print(f"\nDone. Best combined val loss: {best_val:.6f} → {MODEL_OUT}", flush=True)

fig, axes = plt.subplots(1, 2, figsize=(12, 4))
ep = range(1, EPOCHS + 1)
axes[0].plot(ep, history['d'], label='D loss')
axes[0].plot(ep, history['g'], label='G loss')
axes[0].set_xlabel('Epoch'); axes[0].set_title('Training Loss (v10_new)')
axes[0].legend(); axes[0].grid(True)
axes[1].plot(ep, history['val_ssim'], color='green', label='val SSIM')
axes[1].set_xlabel('Epoch'); axes[1].set_title('Validation SSIM (v10_new)')
axes[1].legend(); axes[1].grid(True)
plt.tight_layout()
plt.savefig('./train_v10_new_curves.pdf')
print("Curve saved → train_v10_new_curves.pdf", flush=True)
