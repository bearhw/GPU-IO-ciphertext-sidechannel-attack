...
def connect_monitor() -> socket.socket:
    """Open a fresh connection to the QEMU HMP monitor and consume its
    initial banner/prompt. The returned socket is ready for repeated
    read_page() calls — callers that need many reads (e.g. a whole dict
    build) should open one of these and reuse it instead of paying a new
    connect+banner round trip per read."""
...
def read_page(sock: socket.socket, gpa: int) -> str:
    """Read xp /512gx <gpa> from an already-connected monitor socket
    (see connect_monitor())."""
