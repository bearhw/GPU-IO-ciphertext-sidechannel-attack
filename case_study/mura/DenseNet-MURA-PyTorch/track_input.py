#!/usr/bin/env python3
"""
Input image GPA tracking for DenseNet-MURA.

Guest-side script for tracking input tensor CPU writes using
write_pattern_tracker.c (DST_TRACK) on the host. Sends GPA + timestamps
+ host signals from the guest.

Uses num_workers=0 so image load -> transform -> tensor creation happens
synchronously in the main process. next(iter) must be called after the
host signal so that writes fall within the DST_TRACK active window.

Usage:
    sudo python3 track_input.py --max_iters 10
    sudo python3 track_input.py --max_iters 5 --phase valid
"""
import argparse
import os
import struct
...
def print_tensor_gpa(label, tensor):
    """Print per-page GPA with image/channel labels.

    Tensor shape: (N, 3, 224, 224) float32.
    224*224*4 = 200704 bytes = exactly 49 pages per channel.
    3 channels * 49 = 147 pages per image.
    """
    ...
    pages_per_ch = 49     # 224*224*4 / 4096
    pages_per_img = 147   # 3 * 49
    ch_names = ['R', 'G', 'B']
...
def parse_args():
    p = argparse.ArgumentParser(description="Input image GPA tracker for MURA DenseNet")
...
def main():
    ...
    for i in range(n_iters):
        # 1) Load tensor — pages get mapped in EPT
        data = next(data_iter)
        inputs = data['images'][0]          # (count, C, H, W) on CPU

        # 2) Signal host -> DST_TRACK arm (write-protect pages already in EPT)
        _signal_host("/tmp/d2h_start")
        time.sleep(_SIGNAL_READY_WAIT_S)

        # 3) Write to the same pages again -> triggers write faults
        inputs.mul_(1.0)
